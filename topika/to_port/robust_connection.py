...
class RobustConnection(Connection):
    ...
    def __init__(self, parametres, loop=None, reconnect_interval=DEFAULT_RECONNECT_INTERVAL):
        super(RobustConnection, self).__init__(parametres, loop)
    ...
    def _on_connection_lost(self, future, connection, code, reason):
        for callback in self._on_connection_lost_callbacks:
            callback(self)

        if self._closed:
            return super(RobustConnection, self)._on_connection_lost(future, connection, code, reason)
    ...
    def connect(self):
        ...
        while self._connection is None:
            yield asyncio.sleep(self.reconnect_interval, loop=self.loop)
            result = yield super(RobustConnection, self).connect()
